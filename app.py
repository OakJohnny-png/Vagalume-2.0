...
with aba_gerencia:
    ...
    with col_lateral:
        ...
        with st.expander("📋 Chamados em Andamento", expanded=False):
            ...
            if not os_andamento:
                st.write("Nenhum chamado com a equipa técnica no momento.")
...
with aba_tecnico:
    ...
    # Ordena os mais antigos para o topo da lista
    os_tecnico.sort(key=lambda x: datetime.strptime(x['data'], "%d/%m/%Y %H:%M"))
    
    if not os_tecnico:
        st.info("Nenhuma ordem de serviço pendente para a equipa de rua!")
    else:
...
# ==========================================
with aba_prefeitura:
    st.header("Painel de Gestão e Monitorização")
    ...
    if st.session_state.ordens_servico:
        ...
        st.write("Nenhuma OS registada no momento.")
