...
with aba_gerencia:
    st.markdown(
        """
        <h2 style="white-space: nowrap; font-size: clamp(18px, 3.5vw, 32px); margin-bottom: 20px;">
            Gestão de Chamados e Estoque
        </h2>
        """, 
        unsafe_allow_html=True
    )
    ...
    # --- COLUNA ESQUERDA: CHAMADOS NOVOS ---
    with col_chamados:
        st.markdown(
            """
            <h3 style="white-space: nowrap; font-size: clamp(16px, 2.5vw, 24px); margin-bottom: 10px; color: #444;">
...
with aba_prefeitura:
    st.markdown(
        """
        <h2 style="white-space: nowrap; font-size: clamp(18px, 3.5vw, 32px); margin-bottom: 20px;">
            Painel de Gestão e Monitoramento
        </h2>
        """, 
        unsafe_allow_html=True
    )
    ...
    if st.session_state.ordens_servico:
        ...
        col_m3.metric("Com a Equipe", len(df_os[df_os['status'].isin(['Enviada ao Técnico', 'Em Andamento'])]))
        ...
        st.write("Nenhuma OS registrada no momento.")
