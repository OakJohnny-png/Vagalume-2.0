...
# ---------------------------------------------------------------------------
# Garante que os caminhos sejam resolvidos a partir do diretório do próprio
# script, independentemente de onde o terminal foi aberto.
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
EXCEL_PATH = os.path.join(BASE_DIR, "estoque.xlsx")
HISTORICO_DIR = os.path.join(BASE_DIR, "historico_devolucoes")

# Cria a pasta de histórico caso ainda não exista
...
def carregar_estoque() -> pd.DataFrame:
    ...
    for col in ["Código", "Descrição", "Localização"]:
        if col not in df.columns:
            ...
            st.stop()
    # Remove espaços invisíveis em todas as colunas de texto
...
def carregar_historico_na_sessao(registro: dict):
    ...
    if itens:
        ...
        # Garante que a coluna Quantidade seja numérica
...
# ---------------------------------------------------------------------------
st.sidebar.image(
    "https://img.icons8.com/ios-filled/100/4CAF50/detective.png",
    width=80,
)
st.sidebar.title("Stok Holmes")
st.sidebar.markdown("---")
pagina = st.sidebar.radio(
    "Navegação",
    ["Início", "Finder", "Devolução", "Histórico/Abrir"],
)
st.sidebar.markdown("---")
st.sidebar.caption("NEMA — Obras e Instalações Elétricas")
...
if pagina == "Início":
    ...
    st.info(
        ...
        "- **Histórico/Abrir**: consulte e reabra devoluções anteriores."
    )
...
# ===========================================================================
    ...
    # Duas colunas: Adicionar (esq) | Lista (dir)
    ...
    with col_esq:
        ...
        if not opcoes_encontradas.empty:
            # Monta os rótulos do selectbox: "Código — Descrição"
            rotulos = [
        ...
        if st.button("➕ Adicionar à lista", use_container_width=True):
            if item_selecionado is None:
                ...
                # Verifica se o item já existe na lista (pelo Código)
                ...
                # Mantém a lista ordenada por Descrição
...
# ===========================================================================
# PÁGINA: HISTÓRICO / ABRIR
# ===========================================================================
elif pagina == "Histórico/Abrir":
