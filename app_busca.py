...
def extrair_dados_requisicao(pdf_bytes) -> dict:
    ...
    # Lista de unidades de medida reconhecidas
    _UN = r'PC|KG|MT|CX|PAR|VB|SC|BD|GL|TB|KIT|FD|CJ|JG|RL|LT|UN|M'

    # Padrão 1 (normal): unidade separada por espaço antes da quantidade
    padrao_normal = re.compile(
        r'^(\d{2})\s+(\d{4,6})\s+(.+?)\s+(' + _UN + r')\s+([\d.,]+)',
        re.IGNORECASE
    )

    # Padrão 2 (colado): unidade grudada no final da descrição sem espaço
    # Ex: "...16MMPC 2,00" ou "...CELESCPC 3,00"
    ...
    for linha in linhas:
        ...
        # Tenta padrão normal primeiro (mais preciso)
        ...
        # Tenta padrão com unidade colada (sem espaço antes da UN)
...
def exportar_requisicao_excel(cabecalho: dict, itens: list) -> bytes:
    ...
    # Coluna G (não usada, mas presente por merge do cabeçalho)
...
with tab_requisicoes:
    ...
    # Sub-abas: Manual | Importar PDF
    sub_manual, sub_pdf = st.tabs(["✏️ Criar Manualmente", "📄 Importar via PDF"])
    ...
    with sub_manual:
        ...
        with col_m1:
            man_orcamento = st.text_input("Nº Orçamento / Solicitação", key="man_orcamento")
            man_ped_original = st.text_input("Pedido Original", key="man_ped_original")
            man_revisao = st.text_input("Revisão", key="man_revisao")

            # Cliente: busca no estoque (usa lista de clientes únicos se existir, senão texto livre)
            man_cliente_cod = st.text_input("Código do Cliente", key="man_cliente_cod")
            man_cliente_nome = st.text_input("Nome do Cliente", key="man_cliente_nome")
            man_comprador = st.text_input("Comprador", key="man_comprador")

        with col_m2:
            # Vendedor: texto livre
            man_vendedor = st.text_input("Vendedor", key="man_vendedor")

            # Tipo de Venda: opções fixas
            man_tipo_venda = st.selectbox(
                "Tipo de Venda",
                ["", "Prestação de Serviços", "Venda de Materiais", "Locação", "Outro"],
                key="man_tipo_venda"
            )

            # Departamento: opções fixas
            man_departamento = st.selectbox(
                "Departamento",
                ["", "3 - INSTALAÇÕES ELÉTRICAS", "1 - OBRAS CIVIS", "2 - MANUTENÇÃO", "4 - OUTROS"],
                key="man_departamento"
            )
        ...
        def _montar_cabecalho_manual():
            return {
                "orcamento": man_orcamento or datetime.now().strftime("%d%m%Y%H%M%S"),
                "ped_original": man_ped_original,
                "revisao": man_revisao,
        ...
        with col_sv1:
            if st.button("💾 Salvar Solicitação", use_container_width=True, key="btn_salvar_man"):
                if not st.session_state.req_manual_itens:
                    ...
                    st.success(f"✅ Solicitação salva! Arquivo: **{nome_salvo}**")
        ...
        with col_sv2:
            if st.session_state.req_manual_itens:
                ...
                st.download_button(
                    ...
                    file_name=f"SOL_{man_orcamento or 'nova'}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                    key="btn_dl_man"
                )
                    ...
                    with col_a1:
                        if st.button("💾 Salvar no Sistema", use_container_width=True, key="btn_salvar_req"):
                            if not itens:
                                ...
                                st.success(f"✅ Salvo com sucesso! Arquivo: **{nome_salvo}**")
                    with col_a2:
                        if itens:
                            ...
                            st.download_button(
                                ...
                                file_name=f"REQ_{cabecalho.get('orcamento', 'requisicao')}.xlsx",
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                use_container_width=True,
