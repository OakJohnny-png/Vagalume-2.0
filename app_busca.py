...
SESSION_JSON      = os.path.join(BASE_DIR, "sessoes_ativas.json")
PEDIDOS_PATH      = os.path.join(BASE_DIR, "pedidos.json")
...
def _salvar_sessoes(sessoes: dict):
    with open(SESSION_JSON, "w", encoding="utf-8") as f:
        json.dump(sessoes, f, ensure_ascii=False, indent=4)


def criar_sessao(usuario: str) -> str:
    """Cria token de sessão persistente para o usuário (válido 8 horas)."""
    token = str(uuid.uuid4())
    sessoes = _carregar_sessoes()
    # Remove sessões expiradas
    agora = datetime.now()
    sessoes = {k: v for k, v in sessoes.items()
               if datetime.fromisoformat(v["expira_em"]) > agora}
    sessoes[token] = {
        "usuario": usuario,
        "perfil": get_perfil(usuario),
        "criada_em": agora.isoformat(),
        "expira_em": (agora + timedelta(hours=8)).isoformat(),
    }
    _salvar_sessoes(sessoes)
    return token


def validar_sessao(token: str) -> dict:
    """Valida token de sessão. Retorna dict com dados ou {} se inválido."""
    if not token:
        return {}
    sessoes = _carregar_sessoes()
    s = sessoes.get(token)
    if not s:
        return {}
    if datetime.fromisoformat(s["expira_em"]) < datetime.now():
        del sessoes[token]
        _salvar_sessoes(sessoes)
        return {}
    return s
...
def mostrar_login():
    # CSS completo da tela de login — design Axion dark
    st.markdown("""
    <style>
        ...
        /* Fundo escuro Axion com mesh gradient */
        .stApp {
            background-color: #0F1115 !important;
            background-image:
                radial-gradient(at 0% 0%, rgba(34, 211, 238, 0.06) 0px, transparent 50%),
                radial-gradient(at 100% 100%, rgba(37, 99, 235, 0.08) 0px, transparent 50%) !important;
        }
        .block-container {
        ...
        /* Barra gradiente no topo — cyan/blue Axion */
        .top-gradient-bar {
    ...
    if logo_b64:
        logo_html = f'<img src="data:image/png;base64,{logo_b64}" style="width:400px; height:400px; object-fit:contain;" />'
    else:
    ...
    if entrar:
        if autenticar(usuario_input, senha_input):
            ...
            token = criar_sessao(usuario_norm)
            st.session_state.logado = True
            st.session_state.usuario_logado = usuario_norm
            st.session_state.perfil_logado = get_perfil(usuario_norm)
...
@st.cache_data
def carregar_clientes_dict() -> dict:
    """Retorna {codigo: nome} a partir de clientes.xlsx para auto-preenchimento."""
    if not os.path.exists(REF_CLIENTES):
        return {}
    try:
        df = pd.read_excel(REF_CLIENTES, dtype=str).fillna("")
        result = {}
        for _, row in df.iterrows():
...
def buscar_nome_cliente_por_codigo(codigo: str) -> str:
    """Retorna o nome do cliente pelo código. '' se não encontrado."""
    d = carregar_clientes_dict()
    return d.get(str(codigo).strip(), "")
...
def inicializar_session_state():
    if "logado" not in st.session_state:
        ...
        try:
            ...
            if token:
                info = validar_sessao(token)
                if info:
                    st.session_state.logado = True
                    st.session_state.usuario_logado = info["usuario"]
                    st.session_state.perfil_logado = info["perfil"]
                    st.session_state.session_token = token
        except Exception:
            pass
...
def salvar_em_pedidos_json(dados: dict):
    """Adiciona entrada no arquivo pedidos.json consolidado."""
    pedidos = []
...
def salvar_requisicao(cabecalho: dict, itens: list) -> str:
    ...
    with open(caminho, "w", encoding="utf-8") as f:
        json.dump(dados, f, ensure_ascii=False, indent=4)
    # Grava também no arquivo consolidado pedidos.json
    salvar_em_pedidos_json(dados)
    return nome_arquivo
...
with st.sidebar:
    ...
    <div style='text-align:center; padding: 1rem 0 0.5rem 0;'>
        ...
        <p style='color:#4b5563; font-size:0.65rem; margin:0.2rem 0 0 0; letter-spacing:2px;'>NEMA</p>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")
...
# ===========================================================================
    ...
    with sub_manual:
        ...
        with col_m1:
            ...
            # --- Código do cliente com preenchimento automático do nome ---
            def _on_man_cli_cod_change():
                cod = st.session_state.get("man_cli_cod_input", "").strip()
                nome = buscar_nome_cliente_por_codigo(cod)
                st.session_state["man_cli_nome_auto"] = nome

            if "man_cli_nome_auto" not in st.session_state:
                st.session_state["man_cli_nome_auto"] = ""

            col_cod, col_nome = st.columns([1, 2])
            with col_cod:
                man_cliente_cod = st.text_input(
                    "Código do Cliente",
                    key="man_cli_cod_input",
                    on_change=_on_man_cli_cod_change,
                    placeholder="Ex: 10900",
                )
            with col_nome:
                man_cliente_nome = st.text_input(
                    "Nome do Cliente",
                    value=st.session_state["man_cli_nome_auto"],
                    key="man_cli_nome_display",
                    disabled=True,
                    placeholder="Preenchido automaticamente",
                )
            man_cliente_cod  = st.session_state.get("man_cli_cod_input",  "").strip()
            man_cliente_nome = st.session_state.get("man_cli_nome_auto", "")
            ...
            if pdf_upload is not None:
                with st.spinner("🔍 Lendo e interpretando o PDF..."):
                    try:
                        ...
                        # Auto-preenche Localização vazia a partir do estoque
                        _est = carregar_estoque()
                        preenchidos = 0
                        for item in itens:
                            if not item.get("Localização", "").strip():
                                cod = item.get("Código", "").strip()
                                match = _est[_est["Código"] == cod]
                                if not match.empty:
                                    item["Localização"] = match.iloc[0]["Localização"]
                                    preenchidos += 1

                        st.session_state.req_cabecalho = cabecalho
                        st.session_state.req_itens     = itens
                        msg_loc = f" (**{preenchidos}** localização(ões) preenchida(s) automaticamente)" if preenchidos else ""
                        st.success(f"✅ PDF lido com sucesso! **{len(itens)}** item(ns) encontrado(s).{msg_loc}")
                    except Exception as e:
                        st.error(f"❌ Erro ao processar o PDF: {e}")
                        cabecalho = {}
                        itens     = []

                cabecalho = st.session_state.get("req_cabecalho", {})
                itens      = st.session_state.get("req_itens",     [])
                ...
                if cabecalho or itens:
                    ...
                    st.markdown("---")
                    st.subheader(f"📦 Itens ({len(itens)} item(ns))")
                    if itens:
                    ...
                    # ── Adicionar / remover itens antes de salvar ──────────
                    st.markdown("---")
                    st.subheader("➕ Adicionar / Remover Itens")

                    col_pb, col_pq, col_pu = st.columns([3, 1, 1])
                    with col_pb:
                        pdf_busca = st.text_input(
                    ...
                    if pdf_busca.strip():
                        mask_pdf = (
                            df_estoque["Código"].str.contains(pdf_busca.strip(), case=False, na=False)
                            | df_estoque["Descrição"].str.contains(pdf_busca.strip(), case=False, na=False)
                        )
                        pdf_opcoes = df_estoque[mask_pdf].sort_values("Descrição").reset_index(drop=True)
                        if not pdf_opcoes.empty:
                            rot_pdf = [
                                f"{r['Código']} — {r['Descrição']} [{r['Localização']}]"
                                for _, r in pdf_opcoes.iterrows()
                            ]
                            esc_pdf = st.selectbox("Selecione o item:", rot_pdf, key="pdf_selectbox")
                            idx_pdf = rot_pdf.index(esc_pdf)
                            pdf_item_sel = pdf_opcoes.iloc[idx_pdf]
                        else:
                            st.warning("Nenhum item encontrado.")

                    with col_pq:
                        pdf_quantidade = st.number_input("Quantidade", min_value=1, value=1, step=1, key="pdf_qtd")
                    with col_pu:
                        pdf_un = st.selectbox(
                            "UN",
                            ["PC", "M", "KG", "UN", "CJ", "JG", "RL", "MT", "LT", "CX", "PAR", "KIT"],
                    ...
                    col_add_pdf, col_rem_pdf = st.columns(2)
                    with col_add_pdf:
                        if st.button("➕ Adicionar Item", use_container_width=True, key="btn_add_pdf"):
                            if pdf_item_sel is None:
                                st.error("Selecione um item antes de adicionar.")
                            else:
                                seq_n = len(st.session_state.req_itens) + 1
                                st.session_state.req_itens.append({
                                    "Seq":        f"{seq_n:02d}",
                                    "Localização": pdf_item_sel["Localização"],
                                    "Código":      pdf_item_sel["Código"],
                                    "Descrição":   pdf_item_sel["Descrição"],
                                    "UN":          pdf_un,
                                    "Quantidade":  float(pdf_quantidade),
                                })
                                st.success(f"✅ Item **{pdf_item_sel['Descrição']}** adicionado.")
                                st.rerun()

                    with col_rem_pdf:
                        if st.session_state.req_itens:
                            idx_rem_pdf = st.number_input(
                                "Seq para remover:", min_value=1,
                                max_value=len(st.session_state.req_itens),
                                value=1, step=1, key="pdf_idx_rem",
                            )
                            if st.button("❌ Remover Item", use_container_width=True, key="btn_rem_pdf"):
                                st.session_state.req_itens = [
                                    it for it in st.session_state.req_itens
                                    if it["Seq"] != f"{idx_rem_pdf:02d}"
                                ]
                                for i, it in enumerate(st.session_state.req_itens):
                                    it["Seq"] = f"{i+1:02d}"
                                st.rerun()

                    itens = st.session_state.req_itens  # atualiza referência local
